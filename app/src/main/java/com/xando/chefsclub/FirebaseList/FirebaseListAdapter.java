...
public abstract class FirebaseListAdapter<T, I extends IItem> extends FastItemAdapter<I>
    ...
    private final SparseArray<Boolean> mSyncIndexes;


    public FirebaseListAdapter(@NonNull FirebaseRecyclerOptions<T> options) {
        this(options, null);
    }

    public FirebaseListAdapter(@NonNull FirebaseRecyclerOptions<T> options, List<T> dataList) {
        mSnapshots = options.getSnapshots();
        mIds = new SparseArray<>();
        mSyncIndexes = new SparseArray<>();
    ...
    public void onItemAdded(T data, int pos) {
        String id = getUniqueId(data);
    ...
    public void onItemMoved(int newPos, int oldPos) {
        move(oldPos, newPos);
